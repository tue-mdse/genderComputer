...
The tool also uses the database of first names from all around the world provided together with `gender.c`, an open source C program for name-based gender inference (http://www.heise.de/ct/ftp/07/17/182/). We transform the database (i.e., the `nam_dict.txt` file shipped together with `gender.c`) into a Python dictionary using the [`genderc_python.py`](https://github.com/tue-mdse/genderComputer/blob/master/genderc_python.py) script.
